...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Upstream unconditionally loads spk_dict.pt and calls load_speakers.
        # Local loads speaker assets only when the talker is enabled, allowing
        # thinker-only text ASR models without an audio-output speaker asset.
        # This does not change thinker generation or MAS/CMC math.
        # ------------------------------------------------------------------
        if model.has_talker:
            ...
            if spk_path is None:
                raise ValueError(f"{pretrained_model_name_or_path}/spk_dict.pt does not exist")
    ...
    def generate(
        ...
        **kwargs,
        ...
        # ------------------------------------------------------------------
        # Upstream implementation retained for comparison
        # Upstream validates speaker availability before resolving return_audio.
        # Local validates speaker state only for requested audio output.
        # Text-only generation remains independent of talker speaker metadata.
        # ------------------------------------------------------------------
        if return_audio:
