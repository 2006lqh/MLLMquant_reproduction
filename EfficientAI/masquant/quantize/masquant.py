...
def masquant(
    ...
    # Upstream assumes all three modalities are active during MAS calibration.
    # Local derives active audio/vision masks from dataset_type and uses text
    # scales as the neutral fallback for inactive modalities.
    # This changes calibration selection, not the MAS optimization definition.
    # ----------------------------------------------------------------------
    dataset_type = getattr(args, "dataset_type", "text-audio-vision")
    active_audio = dataset_type in {"audio-only", "text-audio", "vision-audio", "text-audio-vision", "mas_mix_dataset"}
    active_vision = dataset_type in {"vision-only", "text-vision", "vision-audio", "text-audio-vision", "mas_mix_dataset"}
    logger.info(
        f"active modalities: text=True, audio={active_audio}, vision={active_vision}, dataset_type={dataset_type}"
    )
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            ...
            if 'multi_modal_mask' not in kwargs and 'MiniCPM' in args.model:
                ...
                # Upstream: multi_modal_mask_cache.append(kwargs['multi_modal_mask'])
                # Local: remove inactive modality masks before cache and loss use.
                audio_mask, image_mask, text_mask = kwargs['multi_modal_mask']
                if not active_audio:
                    audio_mask = None
                if not active_vision:
                    image_mask = None
                active_mask = (audio_mask, image_mask, text_mask)
                multi_modal_mask_cache.append(active_mask)
                kwargs['multi_modal_mask'] = active_mask
                ...
                if isinstance(module, QuantLinear):
                    for key in pairs.keys():
                        if key in name:
                            ...
                            act_audio = (
                                act_scales[f"{layer_name_prefix}.{i}.{name}.audio_scale"]
                                if active_audio else act_scales[f"{layer_name_prefix}.{i}.{name}.text_scale"]
                            ).to(device=dev, dtype=dtype).clamp(min=1e-5)
                            act_vision = (
                                act_scales[f"{layer_name_prefix}.{i}.{name}.vision_scale"]
                                if active_vision else act_scales[f"{layer_name_prefix}.{i}.{name}.text_scale"]
                            ).to(device=dev, dtype=dtype).clamp(min=1e-5)
                ...
                for j in range(args.nsamples//args.batch_size):
                    ...
                    with traincast():
                        ...
                        if args.loss_multi_modal: 
                            ...
                            # ------------------------------------------------------------------
                            # Upstream implementation retained for comparison
                            # Upstream omits audio_diff when image_mask is absent.
                            # Local accumulates each present modality independently.
                            # This fixes text-audio loss accounting without changing weights.
                            # ------------------------------------------------------------------
                            loss_sum = text_diff.sum()
                            ...
                            if image_mask is not None:
                                ...
                                loss_sum += vision_diff.sum()
                            loss = loss_sum / total_mask
                        else:
