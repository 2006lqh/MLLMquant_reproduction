...
# Purpose of local change:
# - Read calibration audio and transcripts from a local LibriSpeech split.
# - Validate audio/transcript identifiers before text-audio calibration.
# Behavioral difference:
# - Upstream: uses a fixed internal JSONL path for audio calibration.
# - Local: optionally constructs conversations from a supplied local dataset root.
# This is an engineering data-location adaptation; it does not alter MAS/CMC math.
# ----------------------------------------------------------------------
# def prepare_dataset(n_sample: int = 8, data_type: str = 'text-vision') -> list[list[dict]]:
# ----------------------------------------------------------------------
# Active local implementation
# ----------------------------------------------------------------------
def load_librispeech_calibration(root: str, split: str, n_sample: int, include_text: bool) -> list[list[dict]]:
    ...
    selected_ids = sorted(audio_by_id)[:n_sample]
    if len(selected_ids) < n_sample:
        raise ValueError(f"Requested {n_sample} LibriSpeech samples, found {len(selected_ids)}")

    conversations = []
    for utterance_id in selected_ids:
        content = [{"type": "audio", "audio": str(audio_by_id[utterance_id])}]
        if include_text:
            content.append({"type": "text", "text": transcripts[utterance_id]})
...
def prepare_dataset(
    ...
    if data_type == 'text-only':
        ...
        if librispeech_root:
            return load_librispeech_calibration(librispeech_root, split, n_sample, include_text=False)
        ...
        if librispeech_root:
            return load_librispeech_calibration(librispeech_root, split, n_sample, include_text=True)
        dataset_json = '/nas/yuehu/NEW/qwen_compressor/dataset/libri_test_other.jsonl'
        prefix_path = "file:///nas/yuehu/assets/omni_data"
        conversations = []
        with open(dataset_json, "r") as json_file:
    ...
    for batch in batched(calibration_dataset, batch_size, process_func=preprocess_dataset):
        if is_minicpm:
            ...
            if is_qwen_vl == False:
                ...
                # ------------------------------------------------------------------
                # Upstream implementation retained for comparison
                # Upstream: inputs = processor(text=text, images=images, videos=videos, audio=audios, padding=True, return_tensors="pt")
                # Local: disable video-audio coupling for audio-only ASR calibration.
                # This changes processor input preparation only, not quantization math.
                # ------------------------------------------------------------------
                inputs = processor(
                    text=text,
                    images=images,
